...
resultV = libKsj.KSJ_TriggerModeSet(0, 3)
print("KSJ_TriggerModeSet0 error code = %d" % (resultV))
resultV = libKsj.KSJ_TriggerModeSet(1, 3)
print("KSJ_TriggerModeSet1 error code = %d" % (resultV))

resultV = libKsj.KSJ_SetFixedFrameRateEx(0, c_float(8))
print("KSJ_SetFixedFrameRateEx0 error code = %d" % (resultV))
resultV = libKsj.KSJ_SetFixedFrameRateEx(1, c_float(8))
print("KSJ_SetFixedFrameRateEx1 error code = %d" % (resultV))
...
libKsj.KSJ_TriggerModeGet(1, byref(trigermode))
print("trigermode1 =  {0}".format(trigermode.value))
...
pBmpfile0 = create_string_buffer(b'image0.bmp', 64)
pBmpfile1 = create_string_buffer(b'image1.bmp', 64)

while 1:
    retValue1 = libKsj.KSJ_CaptureRgbData(0, pRawData1)

    if retValue1 != 0:
        print("capture1 error code %d" % (retValue1))
    else:
        resultV = libKsj.KSJ_HelperSaveToBmp(pRawData1, nWidth, nHeight, nBitCount, byref(pBmpfile0))
       
    retValue2 = libKsj.KSJ_CaptureRgbData(1, pRawData2) 
    if retValue2 != 0:
        print("capture2 error code %d" % (retValue2))
    else:
        resultV = libKsj.KSJ_HelperSaveToBmp(pRawData2, nWidth, nHeight, nBitCount, byref(pBmpfile1))
