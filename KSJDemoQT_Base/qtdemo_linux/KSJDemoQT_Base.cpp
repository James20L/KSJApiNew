    ...
    ui(new Ui::KSJDemoQT_Base)
{
    ...
    KSJLOG_Init(&hKSJLog,"zhanweilog",0,1);

    KSJLOG_OutputString(hKSJLog, "KSJWARNING_(zhanweilog test!)", 1);

    KSJLOG_OutputError(hKSJLog, 1,  0);

    KSJLOG_OutputLastError(hKSJLog, 1);

    KSJLOG_OutputStringEx(hKSJLog, 0, "zhanwei2 %d",100 );
...
void KSJDemoQT_Base::UpdateInterfaceFunction()
{
    ...
    do{
        ...
        //        ui->FunctionTableWidget->setItem(i, 1, new QTableWidgetItem(QString(szSupport)));
    } while (i--);
}
...
void KSJDemoQT_Base::UpdateDeviceList()
{
    ...
    for (int i = 0; i < m_nDeviceNum; i++)
    {
        ...
        qDebug()<< m_DeviceInfo[i].DeviceType;

        qDebug()<< m_DeviceInfo[i].nSerials;

        qDebug()<<m_DeviceInfo[i].wFirmwareVersion;

        qDebug()<< m_DeviceInfo[i].wFpgaVersion;
        ...
        qDebug()<< btMajVersion;
        qDebug()<< btMinVersion;
        qDebug()<< btFpgaMajVersion;
        qDebug()<< btFpgaMinVersion;
