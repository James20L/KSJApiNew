...
void KSJDemoQT_Matrix::on_PreviewButton_clicked()
{
    ...
    {
        m_pKsjpreviewwidget= new CKSJPreviewWidget(ui->PREVIEWWND_Widget);
        int    nPreviewWndWidth = ui->PREVIEWWND_Widget->width();
        int    nPreviewWndHeight = ui->PREVIEWWND_Widget->height();

        m_pKsjpreviewwidget->setGeometry(0,0,nPreviewWndWidth,nPreviewWndHeight);
...
void KSJDemoQT_Matrix :: CreatSampleImage(unsigned char * pBuf,int nWidth,int nHeight,int nBitCounts)
{
...
void KSJDemoQT_Matrix::on_CaptureButton_clicked()
{
    ...
    if(m_pKsjpreviewwidget != NULL) return;
    ...
    if(info > 0) // is mono
    {
        pImageData = new unsigned char[nCaptureWidth * nCaptureHeight];
        nRet = KSJ_CaptureRawData(m_nDeviceCurSel, pImageData);

//        CreatSampleImage(pImageData,nCaptureWidth,nCaptureHeight,8);

        ShowErrorInfo(nRet);
        format = QImage::Format_Indexed8;

        img = QImage(pImageData, nCaptureWidth, nCaptureHeight, format);  //封装QImage
        img.setColorTable(m_vcolorTable); //设置颜色表
        nCaptureBitCount = 8;


    }else
    {
        pImageData = new unsigned char[nCaptureWidth * nCaptureHeight * (nCaptureBitCount >> 3)];
        KSJ_StartTimeCounter();
        nRet = KSJ_CaptureRgbData(m_nDeviceCurSel, pImageData);

//        CreatSampleImage(pImageData,nCaptureWidth,nCaptureHeight,nCaptureBitCount);


        ShowErrorInfo(nRet);
        int nPassedMs = 0;
        KSJ_CheckTimePassedMilSec(&nPassedMs);

        TCHAR   szElapseTime[32] = { '\0' };
        sprintf(szElapseTime, _T("Elapse: %dms"), nPassedMs);
    ...
    {
         qDebug(" %s %s %d  nCaptureBitCount = %d\n",__FILE__,__FUNCTION__,__LINE__,nCaptureBitCount);
