...
extern int cl_entrance(char* kernelname,int width,int height,unsigned char* inbuf,unsigned char*optput);
...
int KSJ_CreatBufs(int number,T_Fov_param * fovs)
{
    ...
    for(int nCamIndex = 0;nCamIndex < number;nCamIndex++)
    {
        ...
        g_out_buf[nCamIndex] = (unsigned char*)malloc(nCount);
...
void *KSJ_CaptureLoop(void * loopargs)
{
    ...
    printf("bitscount = %d \n",bitscount);
    char windowname[128];
    char windowname_ocl[128];
    sprintf(windowname,"camera serialinloop = %d index = %d",serialinloop,index);
    sprintf(windowname_ocl,"camera ocl serialinloop  = %d index = %d",serialinloop,index);
    ...
    IplImage* img_ocl;
	RGBFLAG = 0;
	img_ocl=cvCreateImage(cvSize(g_fov[index].nWidth,g_fov[index].nHeight),IPL_DEPTH_8U,channelnum);
	img_ocl->imageData = (char*) g_out_buf[index];
	Mat mtx_ocl = cv::cvarrToMat(img_ocl) ;
#endif
    //	Mat	mtx0(img0);
...
#ifdef USING_OPENCL
	gettimeofday(&start,0);
//    	cl_entrance("bayer2",g_fov[index].nWidth,g_fov[index].nHeight,(unsigned char*)g_buf[index],(unsigned char*)g_out_buf[index]);
    	cl_entrance("binarization",g_fov[index].nWidth,g_fov[index].nHeight,(unsigned char*)g_buf[index],(unsigned char*)g_out_buf[index]);
//    	cl_entrance("bayer_image",g_fov[index].nHeight,g_fov[index].nWidth,(unsigned char*)g_buf[index],(unsigned char*)g_out_buf[index]);
    	gettimeofday(&stop,0);
	...
	threshold(mtx_ocl, mtx_ocl, 30, 200.0, CV_THRESH_BINARY);	
    	
	gettimeofday(&stop,0);
    	TimeSubstract(&diff,&start,&stop);
    	printf("cpu  Total time : %d s,%d us\n",(int)diff.tv_sec,(int)diff.tv_usec);


#endif

        KSJ_TRIGGERMODE mode = KSJ_TRIGGER_EXTERNAL;
        CHECK_RET(KSJ_TriggerModeGet(index,&mode));

        printf(" %s %s %d   KSJ_GetTriggerMode mode = %d  nRet =%d \n",__FILE__,__FUNCTION__,__LINE__,mode,CHKRET);
...
HEAD:
    ...
    for(int nThreadNum = 0; nThreadNum <nCamCount;nThreadNum++)
    {
        ...
        paramarray[nThreadNum].out_buf = g_out_buf[nThreadNum];
