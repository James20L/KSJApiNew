...
__kernel void binarization(__global unsigned char *src, __global unsigned char *dst)
...
__kernel void bayer1(__global unsigned char *src, __global unsigned char *dst,int width)
{	int i = get_global_id(0);
	int j = get_global_id(1);
	...
	int downline=j-1;
	dst[j*width+i+1] = src[j-1*width+i];
...
__kernel void b2i(__global unsigned char *src,__read_only image2d_t input, __write_only  image2d_t dst) 
//__kernel void b2i(__global unsigned char *src,__read_only image2d_t input) 
{ 
...
__kernel void  bayer2(__global char * input, __global char* output) 
{
  int width_output = get_global_size(0); 
  int width = get_global_size(0) + 2; 

  float balance[3]={0.1,0.2,0.3};	

   char params[4]={1,2,3,4};
